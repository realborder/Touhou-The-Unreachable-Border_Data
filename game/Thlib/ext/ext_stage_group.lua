...
function stage.group.AddStage(groupname,stagename,item_init,allow_practice)
	...
	if sg~=nil then
		...
		for i,v in ipairs(sg) do
			if v==stagename then flag=true break end
		end
		if not flag then
...
function stage.group.frame(self)
	...
	if lstg.var.lifeleft<=-1 then
		if ext.replay.IsReplay() then
			...
			lstg.tmpvar.pause_menu_text={'Continue','Return to Title','Quit and Save Replay','Restart'}
		end
		lstg.var.lifeleft=0
	end
	--
...
function stage.group.Start(group)
	tuolib.DRP_Sys.Event_StoryModeStart()
...
function stage.group.PracticeStart(stagename)
	tuolib.DRP_Sys.Event_PracticeStart()
