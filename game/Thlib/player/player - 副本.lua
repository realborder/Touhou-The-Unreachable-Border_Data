...
LoadTTF('sc_card','THlib\\UI\\ttf\\SoukouMincho.ttf',30)
...
function DR_Pin:render() --左下角字体渲染
	...
	if player.SC_name~='' then
	    ...
	    if player.SpellTimer1<=90 then
            Render('boss_spell_name_bg',-150,-100-player.SpellTimer1)
            RenderTTF('sc_card',player.SC_name,-149,-149,-110-player.SpellTimer1,-110-player.SpellTimer1,Color(255,0,0,0),'right','noclip')
            RenderTTF('sc_card',player.SC_name,-150,-150,-109-player.SpellTimer1,-109-player.SpellTimer1,Color(255,255,255,255),'right','noclip')
		else
			Render('boss_spell_name_bg',-150,-190)
            RenderTTF('sc_card',player.SC_name,-149,-149,-200,-200,Color(255,0,0,0),'right','noclip')
            RenderTTF('sc_card',player.SC_name,-150,-150,-199,-199,Color(255,255,255,255),'right','noclip')
		end
	end
end
...
--------------------------------------------------------新的符卡设计
			...
			if KeyIsDown'spell' and not lstg.var.block_spell then
				...
				if (lstg.var.bomb>0 and self.death>90) or (self.SpellCardHp==0 and self.nextspell<=0 and self.NextSingleSpell==0 and lstg.var.bomb>0) then
					...
					if self.SpellIndex==1 then self.SC_name='灵符「梦想妙珠」' end
					if self.SpellIndex==2 then self.SC_name='灵符「梦想封印」' end
					if self.SpellIndex==3 then self.SC_name='神灵「梦想封印·寂」' end
					if self.SpellIndex==4 then self.SC_name='珠符「明珠暗投」' end
					if self.SpellIndex==5 then self.SC_name='玉符「阴阳宝玉」' end
					if self.SpellIndex==6 then self.SC_name='宝具「阴阳飞鸟井」' end
