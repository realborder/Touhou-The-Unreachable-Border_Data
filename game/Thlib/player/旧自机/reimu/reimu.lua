...
function reimu_player:newSpell()
    --if self.SpellFlag==true then --若为初次释放符卡
	--    New(player_spell_mask,64,64,200,30,60,30)
	--	New(bullet_cleaner,player.x,player.y, 270, 60, 90, 1)
	--	self.protect=60
	--end
	local deep=min(int(self.KeyDownTimer1/90)+1,4) --用于表示符卡持续按下的阶段，<90时为阶段1，每过90增加一个阶段,最多阶段四
	
    K_dr_SlowSpell=1.25 + K_dr_SpellDmg*lstg.var.dr
    --New(reimu_kekkai,self.x,self.y,K_dr_SlowSpell,3,1,6)
	Print(deep)
...
function reimu_player:render()
    local n=10 local t=0
    if player.PowerDelay1>0 then
	    n=int(self.support)+1
	    t=player.PowerDelay1*255/180 end
	for i=1,6 do
		if self.sp[i] and self.sp[i][3]>0.5 then
		    if i~=n then Render('reimu_support',self.supportx+self.sp[i][1],self.supporty+self.sp[i][2],self.timer*3)
			else SetImageState('reimu_support','',Color(t,255,255,255))
			     Render('reimu_support',self.supportx+self.sp[i][1],self.supporty+self.sp[i][2],self.timer*3)
				 SetImageState('reimu_support','',Color(255,255,255,255)) end
		end
	end
