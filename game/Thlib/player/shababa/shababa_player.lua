...
function shababa_player:init()
	...
	self.lspeed=2
	self.imgs={}
	for i=1,24 do self.imgs[i]='shababa_player'..i end
	self.slist={}
...
function shababa_player:frame()
	if self.nextspell==0 then
		if not(KeyIsDown'slow') then self.slowflag=true self.slowflagtime=4 end
		if not(KeyIsDown'shoot') then self.shootflag=true self.shootflagtime=4 end
		...
		if KeyIsDown'slow'and self.slowflagtime==0 then self.slowflag=false end
		if KeyIsDown'shoot'and self.shootflagtime==0 then self.shootflag=false end
...
function shababa_bomb:frame()
	if self.timer<180 then self.hscale=0.1*self.timer/180 self.vscale=self.hscale
	elseif self.timer<210 then self.hscale=0.1+1*(self.timer-180)/30 self.vscale=self.hscale end
	if self.timer>330 then self.alpha=255-255*(self.timer-330)/30 end
	if self.timer>360 then Del(self) end
	if self.timer==180 then PlaySound('FAa',1.0) PlaySound('enep02',1.0) end
	if self.timer==180 then self.a=256 self.b=256 self.dmg=2.5 New(shababa_b_killer) end
end
...
function shababa_main_ef:init(x,y,rot)
	self.x=x self.y=y self.rot=rot self.img='shababa_bullet_ef' self.layer=LAYER_PLAYER_BULLET+50
	self.vx=4*cos(rot) self.vy=4*sin(rot)
end
...
function shababa_ran_bullet:init(img,x,y,v,angle,dmg,i)
	...
	self.angle=angle
	if i~=1 then self.mute=true end
end
...
function shababa_ran_bullet_ef:init(x,y,rot,img)
	self.x=x self.y=y self.rot=rot self.img=img self.layer=LAYER_PLAYER_BULLET+50
	self.vx=1*cos(rot) self.vy=1*sin(rot)
end

function shababa_ran_bullet_ef:frame()
	if self.timer>=9 then Del(self) end
...
function shababa_junko_laser:init(x,y,angle)
	...
	self.killflag=true
	self.fire=0
	if IsValid(_boss) then self.dmg=0.07
	else self.dmg=0.035 end
end

function shababa_junko_laser:frame()
	if self.timer%8==0 then self.mute=false else self.mute=true end
	if self.timer<10 then self.fire=self.fire+0.1 end
	if self.timer>25 then self.fire=self.fire-0.1 end
	if self.timer==35 then Del(self) end
