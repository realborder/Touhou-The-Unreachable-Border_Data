...
function laser:render()
	...
	if self.w>0 then
		...
		Render(self.img1,self.x,self.y,self.rot,2*self.l1/data[1],2*self.w/7)
		SetImageState(self.img2,b,c)
		Render(self.img2,self.x+self.l1*cos(self.rot),self.y+self.l1*sin(self.rot),self.rot,2*self.l2/data[2],2*self.w/7)
		SetImageState(self.img3,b,c)
		Render(self.img3,self.x+(self.l1+self.l2)*cos(self.rot),self.y+(self.l1+self.l2)*sin(self.rot),self.rot,2*self.l3/data[3],2*self.w/7)
		if self.node>0 then
			c=Color(self._a*self.w/self.w0,self._r,self._g,self._b)
			SetImageState(self.img4,b,c)
			Render(self.img4,self.x,self.y, 18*self.timer,2*self.node/8)
			Render(self.img4,self.x,self.y,-18*self.timer,2*self.node/8)
		end
		if self.head>0 then
			c=Color(self._a*self.w/self.w0,self._r,self._g,self._b)
			SetImageState(self.img5,b,c)
			Render(self.img5,self.x+l*cos(self.rot),self.y+l*sin(self.rot),0,2*self.head/8)
			Render(self.img5,self.x+l*cos(self.rot),self.y+l*sin(self.rot),0,2*0.75*self.head/8)
		end
	end
end
