...
function TUO_Developer_UI:DoPanelFrame(module,panel)
	...
	--焦点逻辑
		if panel.slot == module.cur then
			local keys=self.panel_operation_keys
			local key_pre=self.panel_operation_keypre
			local key_trigger=self.panel_operation_keytrigger
			for k,v in pairs(keys) do
				local pre=key_pre[k]
				local cur = false
				if type(v)=='table' then
					for i,_v in ipairs(v) do
						cur= cur or GetKeyState(_v)
					end
				end
				key_pre[k]=cur
				key_trigger[k] = cur and (not pre)
			end
			local cur=panel.cur
			local total = #(panel.widget)

			if not cur then 
				cur=1 
			elseif type(cur)=='number' then
				if key_trigger.prev then
					cur=cur-1
					if cur<1 then cur=total end
				elseif key_trigger.next then
					cur=cur+1
					if cur>total then cur=1 end
					
				elseif key_trigger.confirm then
					if panel.widget[cur]._event_raw_confirm then 
						(panel.widget[cur]):_event_raw_confirm() 
					elseif panel.widget[cur]._event_confirm then --如果像列表那样可以“进入”这个控件，那就设置一个空函数或者不空的
						(panel.widget[cur]):_event_confirm()
						cur=panel.widget[cur]
					end
				elseif key_trigger.back then
				end
				panel.cur=cur
			else
				if key_trigger.prev then
					if cur._event_prevkey then cur._event_prevkey(cur) end	
				elseif key_trigger.next then
					if cur._event_nextkey then cur._event_nextkey(cur) end	
				elseif key_trigger.confirm then
					if cur._event_confirmkey then cur._event_confirmkey(cur) end	
				elseif key_trigger.back then
					panel.cur=panel.cur.index
				end
			end
		end
	--停留
		if (ux>l and ux<r and abs(uy-y_c)<y_h) then 
			panel.mouse_stay=true 
		else 
			panel.mouse_stay=false 
		end
	--点击
