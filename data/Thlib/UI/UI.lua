...
LoadTTF('WordStyle','THlib\\UI\\ttf\\杨任东竹石体-Heavy.ttf',20)
LoadTTF('PointStyle1','THlib\\UI\\ttf\\杨任东竹石体-Heavy.ttf',30)
LoadTTF('PointStyle2','THlib\\UI\\ttf\\杨任东竹石体-Heavy.ttf',24)
			...
			if player.SpellCardHp>0 then 
			    ...
			    if lstg.var.bomb==0 then SpellName='符卡用尽'
			        ...
			        if player.slow==1 then
				        if lstg.var.bomb==1 then SpellName='珠符「明珠暗投」' end
					    if lstg.var.bomb==2 then SpellName='玉符「阴阳宝玉」' end
					    if lstg.var.bomb==3 then SpellName='宝具「阴阳飞鸟井」' end
				    else
				        if lstg.var.bomb==1 then SpellName='灵符「梦想妙珠」' end
					    if lstg.var.bomb==2 then SpellName='灵符「梦想封印」' end
					    if lstg.var.bomb==3 then SpellName='神灵「梦想封印·寂」' end
				    end	
				end
			end
			RenderTTF('WordStyle',SpellName,518,605.5,352.5,361.5,Color(255,233,255,233),'center','left')
